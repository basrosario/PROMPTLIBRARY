...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // LOGO LETTER TRACING ANIMATION
    // ==========================================
    function initLogoTracing() {
        const logoSpans = document.querySelectorAll('.logo span, .footer-logo span');

        logoSpans.forEach(span => {
            const text = span.textContent;
            const parent = span.parentElement;
            const isFooter = parent.classList.contains('footer-logo');

            // Create SVG container
            const svgNS = 'http://www.w3.org/2000/svg';
            const svg = document.createElementNS(svgNS, 'svg');
            svg.setAttribute('class', 'logo-trace-svg');
            svg.setAttribute('aria-hidden', 'true');

            // Create text element for measuring
            const textEl = document.createElementNS(svgNS, 'text');
            textEl.setAttribute('class', 'logo-trace-text');
            textEl.setAttribute('x', '0');
            textEl.setAttribute('y', '1em');
            textEl.setAttribute('fill', 'var(--primary)');
            textEl.textContent = text;

            // Create the tracing stroke (on top)
            const traceEl = document.createElementNS(svgNS, 'text');
            traceEl.setAttribute('class', 'logo-trace-stroke');
            traceEl.setAttribute('x', '0');
            traceEl.setAttribute('y', '1em');
            traceEl.setAttribute('fill', 'none');
            traceEl.setAttribute('stroke', '#ffffff');
            traceEl.setAttribute('stroke-width', '1');
            ...
            // Measure and set dimensions after DOM insertion
            requestAnimationFrame(() => {
                const bbox = textEl.getBBox();
                svg.setAttribute('width', Math.ceil(bbox.width) + 4);
                svg.setAttribute('height', Math.ceil(bbox.height) + 4);
                svg.setAttribute('viewBox', `${bbox.x - 2} ${bbox.y - 2} ${bbox.width + 4} ${bbox.height + 4}`);

                // Get text length for stroke animation
                const textLength = traceEl.getComputedTextLength();
                traceEl.style.strokeDasharray = textLength;
                traceEl.style.strokeDashoffset = textLength;

                // Start the periodic animation
                const delay = isFooter ? 5000 : 2000; // Stagger footer
                setTimeout(() => startTraceAnimation(traceEl, textLength), delay);
            });
        });
    }

    function startTraceAnimation(traceEl, textLength) {
        const animationDuration = 2000; // 2 seconds to trace
        const pauseDuration = 6000; // 6 seconds pause between traces

        function animate() {
            // Trace in (draw the letters)
            traceEl.style.transition = `stroke-dashoffset ${animationDuration}ms ease-in-out`;
            traceEl.style.strokeDashoffset = '0';

            // After tracing completes, fade out the stroke and reset
            setTimeout(() => {
                traceEl.style.transition = 'stroke-dashoffset 0.5s ease-out, opacity 0.5s ease-out';
