...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SwipeNavigation = {
        // Page flow: Home → Learn Hub → Learn Pages → About → Quiz → Tools Hub → Tools → Home
        pageOrder: [
            '/',
            '/index.html',
            '/learn/index.html',
        ...
        normalizePath(path) {
            ...
            // Remove leading slash for comparison but keep for navigation
            return normalized;
        },
        ...
        getCurrentIndex() {
            ...
            // Find matching page in order
            for (let i = 0; i < this.pageOrder.length; i++) {
                const pagePath = this.pageOrder[i];
                if (currentPath === pagePath ||
                    currentPath === pagePath.slice(1) ||
                    currentPath.endsWith(pagePath) ||
                    (currentPath === '/' && (pagePath === '/' || pagePath === '/index.html'))) {
                    return i;
                }
            }
            return -1;
        ...
        isScrollableElement(target) {
            // Check if target is within a scrollable container
            let element = target;
            while (element && element !== document.body) {
                const style = window.getComputedStyle(element);
                const overflowX = style.getPropertyValue('overflow-x');

                // If element has horizontal scroll, don't capture swipe
                ...
                if (element.tagName === 'INPUT' ||
                    ...
                    element.classList.contains('prompt-builder') ||
        ...
        navigateToPage(index) {
            ...
            if (index < 0) {
                index = this.pageOrder.length - 1;
            } else if (index >= this.pageOrder.length) {
                index = 0;
            }

            const targetPath = this.pageOrder[index];

            // Determine base path for relative navigation
            ...
            // Navigate to target
            const fullPath = basePath + targetPath.slice(1);
            this.showTransition(fullPath);
        },
        ...
        createIndicators() {
            ...
            const prevExists = currentIndex > 0 || true; // Wrap around
            const nextExists = currentIndex < this.pageOrder.length - 1 || true;

            if (prevExists) {
                const prevHint = document.createElement('div');
                prevHint.className = 'swipe-hint swipe-hint--prev';
                prevHint.innerHTML = '<span class="swipe-hint__arrow">‹</span>';
                indicator.appendChild(prevHint);
            }

            if (nextExists) {
                const nextHint = document.createElement('div');
                nextHint.className = 'swipe-hint swipe-hint--next';
                nextHint.innerHTML = '<span class="swipe-hint__arrow">›</span>';
                indicator.appendChild(nextHint);
            }
